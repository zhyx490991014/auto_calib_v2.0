...
对 lidar2camera/auto_calib_v2.0 做了一些修改。

因为在 Jetson 上执行太慢，只有单个cpu在跑，所以用 OMP 优化了各个 for 循环，让 CPU 能跑满。点云分割因为是迭代计算的，没有优化。

因为测试点云是多个文件合并出来的，可能会有重复的点，所以做了一次下采样。但体素滤波在点云太大时会报 overflow，于是改用 octree，对每个叶子节点求质心。

BruteForceSearch 和 RandomSearch 里支持外参左乘 deltaT，因为我觉得应该对“变换到相机坐标系下、用来做投影的点云”进行搜索。不太确定……实际上应该没有影响。

Calibrate() 里增加了对位移的 BruteForceSearch。

---

# 测试

测试是在 Carla 里跑的，所以激光点的 intensity 不准，只跟距离有关。我也试过在 score 中去掉 intensity 的权重，结果还是不理想。

最后结果里外参的 t 不太准，和真值差了十几、二十厘米；角度差得不大; 投影大体能对上。

得分 score 和真值差得很小，只有 0.00 几分，但搜索就是偏离了真值。投影对位移确实不如对旋转敏感？
